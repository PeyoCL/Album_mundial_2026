...
let currentOpenTeam = null; let html5QrcodeScanner = null;
...
async function init() {
    try {
        ...
        if (displaySelect) {
            ...
            displaySelect.addEventListener('change', (e) => { state.displayMode = e.target.value; saveState(); applyCollectionSearch(); if(currentOpenTeam) renderStickersGrid(currentOpenTeam); });
        }
...
function migrateStickerCodes() {
    ...
    for (const [key, value] of Object.entries(state.stickers || {})) {
        if (!value || !value.have) continue; let newKey = key.replace(/-(0?)(\d+)/, '$2'); if (newKey !== key) migrated = true;
...
function toggleSticker(code, ev) {
    ...
    state.stickers[code] = s; saveState(); checkMilestones(); if(currentOpenTeam) renderStickersGrid(currentOpenTeam); updateHomeProgress();
}
...
function checkMilestones() {
    const p = getTotalProgress().percentage; const targets = [25, 50, 75, 100]; if(!state.milestones) state.milestones = {};
...
function renderTeamsGrid(teams) {
    const grid = document.getElementById('teams-grid'); if(!grid) return; grid.innerHTML = ''; let toRender = teams || (window.DATA ? window.DATA.TEAMS : []);
...
function getMinifiedTradeData() {
    ...
    if (window.DATA && window.DATA.TEAMS) {
        ...
        while(bitString.length % 4 !== 0) bitString += "0"; let hexString = "";
...
function showMyQR() { loadQRLibraries(() => {
    ...
    const jsonStr = getMinifiedTradeData(); if (jsonStr.length > 2500) { alert("⚠️ Tienes demasiadas láminas repetidas. Usa el botón 'Copiar Texto'."); return; }
...
function openCameraScanner() { loadQRLibraries(() => { showModal('modal-scanner'); html5QrcodeScanner = new Html5Qrcode("qr-reader"); html5QrcodeScanner.start({ facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } }, (decodedText) => { closeScannerModal(); document.getElementById('match-input').value = decodedText; compareTradesFromText(); }, (errorMessage) => {} ).catch(err => { alert("No se pudo iniciar la cámara."); closeScannerModal(); }); }); }
...
let lastMatchResult = null; function clearMatchInput() { document.getElementById('match-input').value = ''; document.getElementById('match-results-container').style.display = 'none'; lastMatchResult = null; }

function compareTradesFromText() {
    const input = document.getElementById('match-input').value.trim(); if (!input) return;
    try {
        const parsed = JSON.parse(input); let friendState = { profile: { name: parsed.n || 'Tu contacto' }, stickers: {}, legacy: !parsed.m }; let bitString = "";
        if (parsed.m) { for(let i=0; i<parsed.m.length; i++) { bitString += parseInt(parsed.m[i], 16).toString(2).padStart(4, '0'); } }
        if (!window.DATA || !window.DATA.TEAMS) return;
        let index = 0; window.DATA.TEAMS.forEach(team => { team.stickers.forEach(s => { const code = s.code; let isMissing = friendState.legacy ? !(parsed.s && parsed.s[code]) : (bitString[index] === "1"); let count = (parsed.s && parsed.s[code]) ? parsed.s[code] : (isMissing ? 0 : 1); friendState.stickers[code] = { have: !isMissing, count: count }; index++; }); });
